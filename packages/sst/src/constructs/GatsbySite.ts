...
/**
 * The `GatsbySite` construct is a higher level CDK construct that makes it easy to create a Gatsby app.
 * @example
 * Deploys a Gatsby app in the `my-gatsby-app` directory.
 *
 * ```js
 * new GatsbySite(stack, "web", {
 *   path: "my-astro-app/",
 * });
 * ```
 */
export class GatsbySite extends SsrSite {
  ...
  protected createFunctionForEdge(): EdgeFunction {
    ...
    const outputPath = path.resolve(
      path.join(
        ...
        `GatsbySiteFunction-${this.node.id}-${this.node.addr}`
      )
    );
    ...
    if (result.errors.length > 0) {
      ...
      throw new Error(
        `There was a problem bundling the function code for the ${this.id} GatsbySite.`
      );
    }
